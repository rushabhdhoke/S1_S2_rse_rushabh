...
def generate_launch_description():
    ...
    swerve_control_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(PathJoinSubstitution([
            FindPackageShare('rse_shl1_control'), 
            'launch', 
            'swerve_control.launch.py'
        ])),
    )
    ...
    # Load steering and drive controllers after joint_state_broadcaster
    ...
    # Start swerve control after drive controller is loaded
    start_swerve_control_event = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=load_drive_controller,
            on_exit=[swerve_control_launch],
        )
    )
